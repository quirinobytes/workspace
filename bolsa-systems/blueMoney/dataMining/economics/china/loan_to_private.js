...
function gravarCSV (data) {
      if (data.valor_INTEREST_RATE_LAST) {
          csv = '"'+ fields[0] + '"';
          csv+= ',';
          csv+= '"'+ fields[7]+ '"';
          csv+= ',';
          csv+= '"'+ fields[13]+ '"';
          csv+= '\n';
          csv+= '"'+ data.valor_INTEREST_RATE_LAST+ '"';
          csv+= ',';
          csv+= '"'+ data.valor_CASH_RESERVE_RATIO_LAST + '"';
          csv+= ',';
          csv+= '"'+ data.valor_INTERBANK_RATE_LAST + '"';
          csv+= '\n';
          fs.writeFile('../../../csv/all/economics-china-loan_to_private.csv', csv, function(err){ if (err) throw err; });
		  console.log("LOANS= "+JSON.stringify(data));
      }else
		console.log("ATENCAO: arquivo loan_to_private.js LOANS carregou vazio");
}
