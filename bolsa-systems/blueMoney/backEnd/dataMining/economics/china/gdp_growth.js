  ...
  preview: 3,
	extractors: [
		...
		callback: function (err,html,url,response) {
			data = {};
			data.valor_BRASIL_GROWTH_RATE = html.children('tr').eq(0).children('td').eq(1).text();
			data.valor_BRASIL_ANUAL_GROWTH = html.children('tr').eq(1).children('td').eq(1).text();
			...
			fs.writeFile('../../../csv/all/economics-china-gdp_growth.csv', csv, function(err) {
			if (err) throw err;
